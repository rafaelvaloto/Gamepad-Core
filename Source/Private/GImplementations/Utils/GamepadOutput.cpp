...
void FGamepadOutput::OutputDualShock(
    FDeviceContext* DeviceContext)
{
	...
	DeviceContext->BufferOutput[0] =
	    DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth ? 0x11
	                                                                    : 0x05;
	...
	if (DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth)
	{
		const std::uint32_t CrcChecksum = Compute(DeviceContext->BufferOutput, 74);
    ...
    FDeviceContext* DeviceContext)
{
	...
	const size_t Padding =
	    DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth ? 2 : 1;
	DeviceContext->BufferOutput[0] =
	    DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth ? 0x31
	                                                                    : 0x02;
	if (DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth)
	{
...
void FGamepadOutput::SetTriggerEffects(
    unsigned char* Trigger, FGamepadTriggersHaptic& Effect)
{
    ...
    FDeviceContext* DeviceContext)
{
	...
	if (DeviceContext->ConnectionType == EDSDeviceConnection::Bluetooth)
	{
		...
		const std::int32_t CrcChecksum = static_cast<std::int32_t>(
		    Compute(DeviceContext->BufferAudio, CrcOffset));
		DeviceContext->BufferAudio[CrcOffset + 0] =
		    static_cast<unsigned char>((CrcChecksum & 0x000000FF) >> 0UL);
		DeviceContext->BufferAudio[CrcOffset + 1] =
		    static_cast<unsigned char>((CrcChecksum & 0x0000FF00) >> 8UL);
		DeviceContext->BufferAudio[CrcOffset + 2] =
		    static_cast<unsigned char>((CrcChecksum & 0x00FF0000) >> 16UL);
...
const std::uint32_t FGamepadOutput::HashTable[256] = {
...
std::uint32_t FGamepadOutput::Compute(const unsigned char* Buffer,
                                      const size_t Len)
{
	std::uint32_t Result = CRCSeed;
	for (size_t i = 0; i < Len; i++)
	{
		Result = HashTable[static_cast<unsigned char>(Result) ^
